...
- **`Start-Service postgresql-x64-17` иногда падает с `StartServiceFailed` с первого раза**
  ...
  Сверено 2026-06-27.
- **`corepack pnpm -C web deadcode` (knip) падает `RangeError: Array buffer allocation failed`,**
  когда параллельно крутится dev-сервер: oxc-parser не получает память. Не баг кода —
  погасить dev-сервер и повторить. Сверено 2026-07-26.
