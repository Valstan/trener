...
- **Свип 15.08.2026: с диска пропали сразу два артефакта.** Scheduled Task
  `trener-backup-pull` вместе с каталогом `C:\Users\Valstan\bin\` (см. раздел бэкапа выше)
  и — деталь, сообщённая Мозгом письмом 23.08 (пул #192) — **`gh.exe`**: каталог тронут
  ~09:30 того же дня, почерк антивирус-свипа (11.08 тот же почерк по соседнему проекту).
  Вывод для сессий с этой машины: инструмент или задача могут исчезнуть **без ошибки и без
  записи в журнале**. `command not found` здесь — сначала гипотеза «бинарь снесли», а не
  «не установлено»; свежесть `_pull.log` проверять на каждом `/start`.
