...
> Периодические задачи trener на прод-боксе. Дёргают локальные `/cron/*`-эндпоинты с секретом.
...
- **Что:** `trener-rsvp-reminders.timer` → `…​.service` (oneshot) → `curl` на
  `/cron/rsvp-reminders` через loopback-порт сервиса (`PORT` из `trener.env`) с заголовком `x-cron-secret: $CRON_SECRET`.
...
```bash
ssh <box>                                                  # <box> — ssh-алиас прод-бокса из локального ~/.ssh/config
...
- **Что:** `trener-demo-reseed.timer` → `…​.service` (oneshot) → `curl` на
  `/cron/demo-reseed` через loopback-порт сервиса (`PORT` из `trener.env`) с заголовком `x-cron-secret: $CRON_SECRET`.
