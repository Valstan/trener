...
check_canonical() {
  ...
  # Завершающий слэш нормализуем с ОБЕИХ сторон. Next отдаёт canonical главной без
  # него (`https://host`), а не `https://host/` — предмет проверки здесь в том, что у
  # страниц РАЗНЫЕ канонические адреса (G312), а не в форме записи корня.
  #
  # Первая версия сравнивала строки как есть и роняла деплой на исправном проде.
  # Фикстуры этого не поймали, потому что отдавали корень СО слэшем: фикстура
  # моделировала моё допущение, а не поведение Next, и потому благословила сломанную
  # проверку. Режим canonical-root-noslash добавлен ровно затем, чтобы это не повторилось.
  local got_n="${got%/}" exp_n="${expected%/}"

  if [ -z "$got" ]; then
    fail "canonical ${path} — тега нет вовсе"
  elif [ "$got_n" = "$exp_n" ]; then
