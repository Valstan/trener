...
const Page = async () => {
  ...
  return <AppShell title="Дети и группы" tabs={COACH_TABS} active="players"><PlayerGroupList players={players.docs.map((p) => ({ id: p.id, name: p.name, groupId: relId(p.group) }))} groups={groups.docs.map((g) => ({ id: g.id, name: g.name }))} /></AppShell>
}
