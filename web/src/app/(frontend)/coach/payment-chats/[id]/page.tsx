...
const Page = async ({ params }: { params: Promise<{ id: string }> }) => {
  ...
  if (!user) redirect('/login'); if (isPending(user)) redirect('/pending'); if (!isOwner(user)) redirect('/home')
  ...
  return <AppShell title={parentName ?? 'Родитель'} tabs={COACH_TABS} active="payments" back={{ href: '/coach/payment-chats' }}>
