...
const PrivacyPage = async ({ searchParams }: { searchParams: Promise<{ branch?: string }> }) => {
  ...
  return (
    <main className="page" style={{ maxWidth: 760 }}>
      ...
      <Section n={6} title="Передача, уведомления и статистика посещений">
        <p>
        ...
        <p>
          На <strong>публичной странице сайта</strong> (главная, до входа в приложение) работает
          счётчик Яндекс.Метрики: он собирает обезличенную статистику посещений — факт визита,
          источник перехода, тип устройства — и использует cookie. Персональные данные детей и
          родителей ему не передаются. В личных кабинетах (расписание, чаты, оплата) счётчика нет.
        </p>
      </Section>
